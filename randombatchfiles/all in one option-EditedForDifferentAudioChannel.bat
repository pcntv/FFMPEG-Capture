...
:original
ffmpeg -i %1 -ss %ss% -to %to% -map v:0 -map a:1 -c:v libx264 -profile:v main -level 3.1 -pix_fmt yuv420p -s 1280x720 -crf 18 -maxrate 4000000 -bufsize 4000001 -c:a aac -b:a 128k -af dynaudnorm=500:13:0.90:10:0.16:0:0:0:8 -movflags +faststart "%readyfolder%\%~n1.mp4"
echo "ALL DONE Upload %~n1 to Brightcove"
pause 
exit
